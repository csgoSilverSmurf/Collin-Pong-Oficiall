...
wKey = pygame.transform.scale(wKey,(100,100))
sKey = pygame.image.load('s.jpg')
sKey = pygame.transform.scale(sKey,(100,100))
...
#this function is used when red OR blue wins
def textObjects(text,font):
    #black font
    textSurface = font.render(text,True,BLACK)
    return textSurface,textSurface.get_rect()

#this function is also used when red OR blue wins
...
def instructions():
    ...
    while check:
        ...
        textRect.center = (335, 150)
        gameDisplay.blit(textSurf, textRect)

        largeText = pygame.font.Font("freesansbold.ttf", 25)
        textSurf, textRect = textObjects("DOWN", largeText)
        textRect.center = (335, 250)
        gameDisplay.blit(textSurf, textRect)
        
        #draw yellow rectangle and black rectangle with no fill for an outline
        pygame.draw.rect(gameDisplay,YELLOW,(90,450,300,75))
        pygame.draw.rect(gameDisplay, BLACK ,[90,450,300,75],4) #4 value for outline
        
        largeText = pygame.font.Font("freesansbold.ttf", 16)
        textSurf, textRect = textObjects("First to 7 points wins", largeText)
        textRect.center = (240,490)
        gameDisplay.blit(textSurf, textRect)

        pygame.draw.rect(gameDisplay,YELLOW,(480,450,300,75))
        pygame.draw.rect(gameDisplay, BLACK ,[480,450,300,75],4)
        
        largeText = pygame.font.Font("freesansbold.ttf", 16)
        textSurf, textRect = textObjects("Reflect the ball with your paddle", largeText)
        textRect.center = (630, 490)
        gameDisplay.blit(textSurf, textRect)

        pygame.draw.rect(gameDisplay,YELLOW,(880, 450, 300,75))
        pygame.draw.rect(gameDisplay, BLACK ,[880, 450, 300,75],4)
        
        largeText = pygame.font.Font("freesansbold.ttf", 15)
        textSurf, textRect = textObjects("Score on the opposite side to gain points", largeText)
        textRect.center = (1029, 490)
        gameDisplay.blit(textSurf, textRect)
...
def mainLoop():
    ...
    while gameExit == False:
        ...
        gameDisplay.fill(color)
        #draw mid line
        pygame.draw.line(gameDisplay,BLACK,(DISPLAY_WIDTH/2,0),(DISPLAY_WIDTH/2,DISPLAY_HEIGHT),5)
        #draw mid circle (not to be confused for ball)
        pygame.draw.circle(gameDisplay,BLACK,(DISPLAY_WIDTH//2,DISPLAY_HEIGHT//2),60,5)
        #green rectangle
        ...
        #if hit BLUE paddle
        #if ball must land within paddle WIDTH and HEIGHT in order to reflect
        if (ballStartX+ballSpeedX >= x1) and (ballStartX+ballSpeedX <= x1+paddle1_WIDTH) and ((y1 <= ballStartY+ballSpeedY+ballRadius) and (ballStartY+ballSpeedY-ballRadius <= y1+paddle1_HEIGHT)):
            ballSpeedX *= -1
            ...
            ballSpeedY = -ballSpeedY/((6*ballRadius)/8)
            pygame.mixer.music.load("ball.wav")
            pygame.mixer.music.set_volume(1)
        ...
        #if ball must land within paddle WIDTH and HEIGHT in order to reflect
        if (ballStartX+ballSpeedX <= x2+paddle2_WIDTH) and (ballStartX+ballSpeedX >= x2) and ((y2 <= ballStartY+ballSpeedY+ballRadius) and (ballStartY+ballSpeedY-ballRadius <= y2+paddle2_HEIGHT)):
        ...
        if ballStartX < 0:
            ...
            #coin like sound when player wins point
            pygame.mixer.music.load("point.wav")
            pygame.mixer.music.set_volume(1)
